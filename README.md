...
- kolin에서도 java와 동일하게 중첩클래스를 만들 수 있다.
  - 강의에서는 실무에서 잘 사용하지 않는다고 하였으며, 필자 역시 실무에서 개발할 때 딱히 사용하지 않았던 것 같다.
- 클래스안에 class 키워드만 사용하면 만들면 외부 클래스를 참조 하지 않는다.
  - 이 방법이 권장된다.
- 클래스안에 inner 키워드와 class 키워드를 함께 사용하면 외부 클래스를 참조 가능하다.
  - 외부를 참조하게 되면 메모리누수가 발생할 수 있다.
  - 내부클래스의 직렬화형태가 명확하지 않아 직렬화에 제한이 있다.
